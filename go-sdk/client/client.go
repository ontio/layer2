...
func (this *ClientMgr) GetLayer2State(height uint32) (*sdkcom.Layer2State, []keypair.PublicKey, error) {
	client := this.getClient()
	if client == nil {
		return nil, nil, fmt.Errorf("don't have available client of ontology")
	}
	data, err := client.getLayer2State(this.getNextQid(), height)
	if err != nil {
		return nil, nil, err
	}
	newdata := string(data)
	newdata = newdata[1: len(newdata) - 1]
	data, _ = hex.DecodeString(newdata)
	return utils.GetLayer2State(data)
}
