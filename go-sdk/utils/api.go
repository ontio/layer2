...
func GetLayer2State(data []byte) (*sdkcom.Layer2State, []keypair.PublicKey, error) {
	state := &sdkcom.Layer2State{}
	source := common.NewZeroCopySource(data)
	err := state.Deserialization(source)
	if err != nil {
		return nil, nil, err
	}
	pksLen, _, _, _ := source.NextVarUint()
	pks := make([]keypair.PublicKey, pksLen)
	for i := 0;i < int(pksLen);i ++ {
		temp, _, _, _ := source.NextVarBytes()
		pk, _ := keypair.DeserializePublicKey(temp)
		pks[i] = pk
	}
	return state, pks, nil
}
